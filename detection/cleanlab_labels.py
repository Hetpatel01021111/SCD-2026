...
from utils.data_loader import PoisonedDataset, get_train_transform
...
def detect_label_issues_oof(base_dataset, observed_labels, model_factory=build_resnet18,
    ...
    for held_out in fold_ids:
        ...
        train_loader = DataLoader(
            Subset(base_dataset, train_ids), batch_size=config.BATCH_SIZE,
            shuffle=True, num_workers=config.NUM_WORKERS,
            pin_memory=config.PIN_MEMORY, persistent_workers=config.NUM_WORKERS > 0,
        )
        test_loader = DataLoader(
            Subset(base_dataset, held_out), batch_size=config.EVAL_BATCH_SIZE,
